...
export class OutputManager {
    ...
    private defaultAnsiFg = ["green", "low"];
    private defaultAnsiBg = ["black", "low"];

    constructor(pMessage: Message, pOutputWin: OutputWin) {
        this.pMessage = pMessage;
        this.pOutputWin = pOutputWin;        

        this.pMessage.sub('load_layout', this.handleLoadLayout, this);
        this.pMessage.sub('change_default_color', this.handleChangeDefaultColor, this);
    ...
    private handleLoadLayout() {
        ...
        this.targetWindows = [this.pOutputWin];
        this.target = this.pOutputWin;

        this.initColor();
    }

    public output_done () {
        this.target.output_done();
    };
    ...
    public pushMxpElem(elem) {
        this.target.push_elem(elem);
    }

    public popMxpElem() {
        return this.target.pop_elem();
    }

    public handleText(data) {
        this.target.add_text(data);
    }

    private setFgColor(color) {
        this.fgColor = color;
        this.target.set_fg_color(color);
    }

    private setAnsiFg(color) {
        this.ansiFg = color;
        this.setFgColor(this.ansiFg ? (colors[this.ansiFg[0]][this.ansiFg[1]]) : null);
    }

    private setBgColor(color) {
        this.bgColor = color;
        this.target.set_bg_color(color);
    }

    private setAnsiBg(color) {
        this.ansiBg = color;
        this.setBgColor(this.ansiBg ? (colors[this.ansiBg[0]][this.ansiBg[1]]) : null);
    }

    public getFgColor() {
        return this.fgColor || colors[this.defaultAnsiFg[0]][this.defaultAnsiFg[1]];
    }

    private getBgColor() {
        return this.bgColor || colors[this.defaultAnsiBg[0]][this.defaultAnsiBg[1]];
    }
...
//        set_ansi_fg([colors.green, 'low']);
    ...
    public handle_xterm_escape(data) {
        var splt = data.split(';');
    ...
    public handleAnsiGraphicCodes(codes: Array<string>) {
        ...
        for (var i=0; i < codes.length; i++) {
            ...
            if (code == 1) {
                ...
                if (this.ansiReverse) {
                    if (new_bg || this.ansiBg || !this.bgColor) {
                        new_bg = new_bg || this.ansiBg || this.defaultAnsiBg.slice();
                        new_bg[1] = 'high';
                    }
                } else {
                    if (new_fg || this.ansiFg || !this.fgColor) {
                        new_fg = new_fg || this.ansiFg || this.defaultAnsiFg.slice();
                        new_fg[1] = 'high';
                    }
                }
                continue;
            ...
            if (code == 7) {
                ...
                var fg = new_fg || this.ansiFg || this.defaultAnsiFg.slice();
                var bg = new_bg || this.ansiBg || this.defaultAnsiBg.slice();
                new_fg = bg;
                new_bg = fg;
            ...
            if (code >= 30 && code <= 37) {
                ...
                var color_name = ansiFgLookup[code];
                new_fg = new_fg || this.defaultAnsiFg.slice();
                new_fg[0] = color_name;
                continue;
            }
            ...
            if (code >= 40 && code <= 47) {
                ...
                var color_name = ansiBgLookup[code];
                new_bg = new_bg || this.defaultAnsiBg.slice();
                new_bg[0] = color_name;
                continue;
            }
        }
    ...
    private setDefaultAnsiFg (color_name, level) {
        ...
        this.defaultAnsiFg = [color_name, level];
        $('.output_text').css('color', colors[color_name][level]);
    }

    private handleChangeDefaultColor(color_name) {
        var level = "low";

        this.setDefaultAnsiFg(color_name, level);
        this.saveColorCfg();
    };
