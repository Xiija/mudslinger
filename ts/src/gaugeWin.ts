...
export class GaugeWin {
    ...
    private load_layout() {
        ...
        $("#tnl_bar").jqxProgressBar({
            ...
            animationDuration: 0,
            renderText: (text) => {
                let tnl = this.msdp_vals["EXPERIENCE_TNL"] || 0;
                let max = this.msdp_vals["EXPERIENCE_MAX"] || 0;
        ...
        for (let k in this.update_funcs) {
            this.update_funcs[k]();
        }
    }
    ...
    private create_update_funcs() {
        this.update_funcs["HEALTH"] = () => {
            let val = this.msdp_vals["HEALTH"] || 0;
            let max = this.msdp_vals["HEALTH_MAX"] || 0;
            if ( !max || max === 0) { return; }
        ...
        this.update_funcs["MANA"] = () => {
            let val = this.msdp_vals["MANA"] || 0;
            let max = this.msdp_vals["MANA_MAX"] || 0;
            if ( !max || max === 0) { return; }
        ...
        this.update_funcs["MOVEMENT"] = () => {
            let val = this.msdp_vals["MOVEMENT"] || 0;
            let max = this.msdp_vals["MOVEMENT_MAX"] || 0;
            if ( !max || max === 0) { return; }
        ...
        this.update_funcs["OPPONENT_HEALTH"] = () => {
            let val = this.msdp_vals["OPPONENT_HEALTH"] || 0;
            let max = this.msdp_vals["OPPONENT_HEALTH_MAX"] || 0;
            if ( !max || max === 0) { return; }
        ...
        this.update_funcs["EXPERIENCE_TNL"] = () => {
            let val = this.msdp_vals["EXPERIENCE_TNL"] || 0;
            let max = this.msdp_vals["EXPERIENCE_MAX"] || 0;
            if ( !max || max === 0) { return; }
